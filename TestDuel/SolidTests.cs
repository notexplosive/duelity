...
namespace TestDuel
{
    public class SolidTests
    {
        ...
        {
            var level = new Level(new Corners(new Point(-2, -2), new Point(2, 2)));
            level.PutEntityAt(new Point(1, 1), new EntityTemplate(new SolidTag(SolidTag.Type.Static)));
        ...
        {
            var level = new Level(new Corners(new Point(-10, -10), new Point(10, 10)));
            var pushable = level.PutEntityAt(Point.Zero, new EntityTemplate(new BlockProjectileTag(), new Hittable(Hittable.Type.PushOnHit)));
            var solidProvider = new LevelSolidProvider(level);
